...
class Engine:
    ...
    def process_batch(self, batch: List[str]) -> List[JsonCompletion]:
        ...
        batch_log_probs = [None] * len(batch)

        queue = deque([(i, text) for i, text in enumerate(batch)])

        # Process the queue until all outputs are valid
        while queue:
            indices = []
            prompts = []
            while queue:
            ...
            for i, raw_output in zip(indices, raw_outputs):
                # print(f"{i = }, {raw_output.outputs[0].text = }")

                json_output = parse_json(raw_output.outputs[0].text)
                log_probs = raw_output.outputs[0].logprobs
    ...
    def __call__(self) -> Generator[JsonCompletion, None, None]:
        for batch in tqdm(self.dataset, desc="Processing batches"):
            json_completions, batch_log_probs = self.process_batch(batch)
